...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        audio_feats, wav_lens = batch.audio_feat
        # video_feats, vid_lens = batch.video_feat
        # load text
        tokens_bos, _ = batch.tokens_bos
        audio_feats, wav_lens = audio_feats.to(self.device), wav_lens.to(self.device)  # [B, T, C]
        # video_feats, vid_lens = video_feats.to(self.device), vid_lens.to(self.device)  # [B, T, C]

        # Forward fusion module
        video_feats = torch.zeros_like(audio_feats)[:, int(audio_feats.shape[1]/2):].to(self.device)
        feats = self.modules.fusion(audio_feats, video_feats)
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            self.scaler.unscale_(self.fusion_optimizer)
            self.scaler.unscale_(self.adam_optimizer)

            if self.check_gradients(loss):
                self.scaler.step(self.fusion_optimizer)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            sb.nnet.schedulers.update_learning_rate(
                self.fusion_optimizer, new_lr_model
            )
    ...
    def init_optimizers(self):
        ...
        self.fusion_optimizer = self.hparams.fusion_opt_class(
            self.hparams.fusion.parameters()
        )
        ...
        if self.checkpointer is not None:
            self.checkpointer.add_recoverable("fusion_opt", self.fusion_optimizer)
...
def dataio_prepare(hparams):
    ...
    if hparams["sorting"] == "ascending":
        ...
        train_data = train_data.filtered_sorted(sort_key="frame1")
        # when sorting do not shuffle in dataloader ! otherwise is pointless
        hparams["train_dataloader_opts"]["shuffle"] = False

    elif hparams["sorting"] == "descending":
        train_data = train_data.filtered_sorted(
            sort_key="frame1", reverse=True
        )
        # when sorting do not shuffle in dataloader ! otherwise is pointless
        hparams["train_dataloader_opts"]["shuffle"] = False
    ...
    valid_data = valid_data.filtered_sorted(sort_key="frame1")
    ...
    @sb.utils.data_pipeline.provides("audio_feat")
    def audio_pipeline(audio):
        if hparams["add_noise"]:
            snr_db = hparams["snr_db"]
            audio_path = audio.replace("clean", f"SNR_{snr_db}dB")
        else:
            audio_path = audio
    ...
    @sb.utils.data_pipeline.takes("video")
    @sb.utils.data_pipeline.provides("video_feat") 
    def video_pipeline(video):
        video_feat = torch.load(video)
        return video_feat
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "audio_feat", "video_feat", "wrd", "char_list", "tokens_bos", "tokens_eos", "tokens"],
    )
